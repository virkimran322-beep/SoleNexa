...
- A physical stock count can include several material, bin and optional lot lines on one sheet. Save the sheet, submit it for review, and approve all lines together after checking the physical quantities. If stock changes after the sheet was created, approval is blocked and the count must be repeated.
- An Accountant can approve either Weighted average or FIFO in Settings. The valuation report shows current quantity, average rate and value using landed purchase rates. Completed months can then be closed; new inventory entries dated inside a closed month are blocked.
- When issuing or scrapping material against a PO, SoleNexa shows the saved BOM quantity and current issued/returned/scrap totals. PO details show planned, issued, returned, scrap, actual and provisional WIP quantities. Scrap is a separate stock movement and cannot consume reserved stock.
- Quality inspections are recorded from the Quality inspections page. Choose Incoming material, In-process work or Final finished output, record accepted/rejected quantities and a disposition. Rejected quantity needs a defect description; submit the draft for Manager/Owner approval.
