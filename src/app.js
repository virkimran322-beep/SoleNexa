...
function settings() {
  return (
    ...
    `<div class="setting">${panel("Factory profile", `<div class="panel-body"><p><strong>${esc(state.config.companyName || "Factory workspace")}</strong><br>${esc(state.config.owner || "")}<br>${esc(state.config.contact || "")}<br>${esc(state.config.address || "")}</p>${can("company-profile") ? btn("Edit factory profile", "company-profile", "", true) : ""}<p class="hint">Changes apply to future screens and print documents. Historical records keep their original snapshots.</p></div>`)}${panel("Inventory valuation policy", `<div class="panel-body"><p>${badge(state.config.inventoryValuation === "unconfigured" ? "Accountant approval required" : state.config.inventoryValuation, state.config.inventoryValuation === "unconfigured" ? "amber" : "green")}</p><p class="hint">Landed cost is captured on each purchase. The accountant can approve weighted-average or FIFO valuation. Closed periods block later backdated inventory entries.</p>${can("inventory-valuation") ? btn("Approve valuation method", "inventory-valuation", "", true) : ""}${can("inventory-close") ? btn("Close completed period", "inventory-close") : ""}</div>`)}${panel("Inventory valuation report", `<div class="panel-body">${state.inventoryValuation ? `<p><strong>${esc(state.inventoryValuation.method)}</strong> · As of ${esc(state.inventoryValuation.asOf)} · Total value ${money(state.inventoryValuation.totalValue)}</p>${table(["Material","Quantity","Average rate","Value"], state.inventoryValuation.materials.map((line) => `<tr><td>${esc(line.materialName)}<small>${esc(line.unit)}</small></td><td>${qty(line.quantity)}</td><td>${money(line.averageRate)}</td><td>${money(line.value)}</td></tr>`))}` : '<p class="muted">Choose a valuation method to view inventory value.</p>'}</div>`)}${panel("Offline application", `<div class="panel-body"><p>SoleNexa stores factory records locally on this computer and works without internet.</p></div>`)}${panel("Production departments", `<div class="panel-body"><div class="checks">${state.department.map((d) => `<span class="tag-with-action">${badge(d.name)}${d.active === false ? badge("Inactive", "amber") : ""}${can("department-revise") ? btn("Edit", "department-revise", d.id) + btn("History", "department-history", d.id) : ""}</span>`).join("")}</div>${btn("+ Add department", "department")}<p class="hint">New departments can be selected on future POs. Existing POs keep their saved department names.</p></div>`)}${panel(
      "Thermal printing",
...
function form(action, id) {
  ...
  if (action === "inventory-valuation") {
    title = "Approve inventory valuation method";
    fields = select("method", "Valuation method", [["weighted-average", "Weighted average"], ["fifo", "FIFO"]], state.config.inventoryValuation === "fifo" ? "fifo" : "weighted-average") + '<p class="hint full">This policy applies to future valuation reports. Landed purchase rates and historical PO snapshots remain unchanged.</p>';
  }
  if (action === "inventory-close") {
    title = "Close completed inventory period";
